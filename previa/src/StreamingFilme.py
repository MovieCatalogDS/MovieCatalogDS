...
''' 
    ...
    Essa func depende do arquivo "filme.csv"
...
def providers_json_to_list(json, movie_id):
    ...
    try:
        ...
        print("Bem vindo ao mundo! OwO \nId do filme que parou: " + str(movie_id))
...
def save_json(data, name):
    ...
    with open('data/temp/' + str(name) + '.json', 'wb') as f:
        f.write(content)
...
def get_movie_providers(movie_id):
    providers_json = ''
    try:
        with open('data/temp/' + str(movie_id) + '.json', 'r') as movie_providers:
...
def get_all_providers():
    ...
    return pd.DataFrame(providers_tuples, columns=['nome'])
